...
class PredictSense:
    ...
    def __init__(self):
        ...
        self.__saveData = "/api/data/pull" # '/api/notebook/api/add_data'
        self.__featuredetails__ = {}
        self.__sharingFunction = \
            "/api/projects/:projectId/feature_repo/" + \
                "share_features/:userId/share_from_py?type=function"
        self.__registerFunction = "/api/feature_repo/register_function"
        import os
        port = 3000 if "PORT" not in os.environ else os.environ['PORT']
        self.__ui_url = "http://localhost:" + str(port)
        self._js_ = '''
    ...
    def load_project_details(self, file_filter=".csv"):
        ...
        available_files = []
        lst = []
        filters = r"(csv|xls|xlsx|pkl|json|feather|parquet){1}$" \
            if file_filter == "all" else (
                r"" + file_filter + r"$")
        for root, _, files in os.walk(os.path.abspath(os.path.pardir)):
    ...
    def save_data(self, df, file_name):
        ...
        df.to_csv(os.path.abspath(self.__current_file), index=False)
        request_obj = {
            'filename': os.path.abspath(self.__current_file),
            'projectId': self.__getdetails__['_id'],
            'createdBy': self.__getdetails__['user_id'],
            'fileSize': int(df.memory_usage().sum()), 
            'source': 'upload', 
            'fileEncoding': 'utf_8', 
            'filePath': os.path.abspath(self.__current_file)
        }
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                print (
                    "File saving started.\n" + \
                    "You should see it in the data tab in some time.")
    ...
    def list_uploaded_datasets(
        self, file_filter=".csv", file_source='file', print_details=True):
        """
        ...
        file_filter: str
            Filter for the files to be listed. 
            Allowed values are .csv, .xlsx, .json or all.

        file_source: str
            Filter for the files to be listed by data source. 
            Allowed values are file, mssql, mysql or all.

        print_details: bool
            Whether to print the details related to the files
    ...
    def update_project_details(self, NOTEBOOK_URL):
        """
        Update notebook url. Cannot be used directly by user.
        ...
        try:
            ...
            parsed = urlparse.urlparse(NOTEBOOK_URL)
            parsed = parse_qs(parsed.query)
            if self.__getdetails__ is None:
                self.__getdetails__ = {}
            self.__getdetails__["_id"] = parsed['pid'][0]
            self.__getdetails__["user_id"] = parsed['uid'][0]
            file_list = self.list_uploaded_datasets(
                file_filter='pkl', file_source='all', print_details=False)
            self.__getdetails__["fileEncoding"] = \
                file_list['fileEncoding'].dropna().values[0]
        except:
            print ("Make sure you are running the notebook in PredictSense.")
...
def normalizing_dataframe_columns(columns):
    ...
    import re
    norm_columns = columns.str.replace('.', '')
    norm_columns = [re.sub(r"\s+", "_", str(x)).lower().strip() for x in
                norm_columns]
